    ...
    <div className="min-h-screen bg-[#f7f8fc] text-[#172b4d]">
      <header className="sticky top-0 z-10 flex h-16 items-center gap-6 border-b border-[#e5e8ef] bg-[rgba(255,255,255,0.92)] px-6 backdrop-blur-xl max-[800px]:gap-3 max-[800px]:px-3.5">
        <div className="flex min-w-52 items-center gap-2.5 text-lg font-extrabold max-[800px]:min-w-0">
          <span className="grid size-8 place-items-center rounded-[9px] bg-[#635bff] text-white">T</span>
          <span className="max-[800px]:hidden">Trello Mini</span>
        </div>

        <label className="relative w-[min(420px,42vw)] max-[800px]:hidden">
          <span className="absolute top-1/2 left-3.5 -translate-y-1/2 text-[#6b778c]" aria-hidden="true">⌕</span>
          <input
        ...
        <div className="ml-auto flex items-center gap-2.5">
          <button className={iconButtonClasses} type="button" aria-label="Tạo mới">＋</button>
          <button className={iconButtonClasses} type="button" aria-label="Thông báo">♧</button>
          ...
          <details className="group relative">
            ...
            <div className="absolute top-[calc(100%+12px)] right-0 z-30 w-[min(290px,calc(100vw-28px))] rounded-[14px] border border-[#e1e4ea] bg-white p-2.5 shadow-[0_18px_48px_rgba(23,43,77,0.20)] before:absolute before:-top-1.5 before:right-3.5 before:size-[11px] before:rotate-45 before:border-t before:border-l before:border-[#e1e4ea] before:bg-white before:content-['']">
              <p className="mx-2 mt-0.5 mb-2.5 text-[10px] font-extrabold tracking-[0.08em] text-[#8993a4] uppercase">Tài khoản</p>

              <div className="flex items-center gap-[11px] px-2 pt-[7px] pb-2.5">
                <span className="grid size-[42px] shrink-0 place-items-center rounded-full bg-[#dcd9ff] text-xs font-extrabold text-[#4037aa]" aria-hidden="true">HN</span>
                <div className="min-w-0">
                  <strong className="mb-[3px] block text-[13px] text-[#263752]">Hải Nguyễn</strong>
                  <span className="block max-w-[190px] overflow-hidden text-[11px] text-ellipsis whitespace-nowrap text-[#7a869a]">hai.nguyen@example.com</span>
                </div>
              </div>

              <div className="mx-1 my-1.5 h-px bg-[#eef0f4]" />

              <button className="flex w-full cursor-pointer items-center gap-[11px] rounded-[9px] border-0 bg-transparent p-2.5 text-left text-[#ae2e24] hover:bg-[#ffebe6] focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[rgba(201,55,44,0.20)]" type="button">
                <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[#ffebe6] text-[19px] font-extrabold" aria-hidden="true">↪</span>
                <span>
                  <strong className="block text-[13px]">Đăng xuất</strong>
                  <small className="mt-0.5 block text-[10px] font-medium text-[#7a869a]">Thoát khỏi tài khoản hiện tại</small>
                </span>
              </button>
            </div>
          </details>
      ...
      <div className="grid min-h-[calc(100vh-64px)] grid-cols-[240px_minmax(0,1fr)] max-[800px]:block">
        <Sidebar />
        <main className="min-w-0 px-8 pt-7 pb-10 max-[800px]:px-4 max-[800px]:pt-[22px] max-[800px]:pb-8">{children}</main>
      </div>
    </div>
  );
}
  ...
  return (
    <aside className="border-r border-[#e5e8ef] bg-white px-4 py-6 max-[800px]:hidden">
      <nav aria-label="Điều hướng chính">
        <p className="mx-3 mt-[18px] mb-2 text-[11px] font-extrabold tracking-[0.08em] text-[#8993a4] uppercase">Không gian làm việc</p>
        <button className={navButtonClasses} type="button"><span className="w-5 text-center">⌂</span>Tổng quan</button>
        <button className={`${navButtonClasses} bg-[#efeeff] font-bold text-[#5147d9] hover:bg-[#efeeff]`} type="button"><span className="w-5 text-center">▦</span>Bảng công việc</button>
        <button className={navButtonClasses} type="button"><span className="w-5 text-center">✓</span>Công việc của tôi</button>

        <p className="mx-3 mt-[18px] mb-2 text-[11px] font-extrabold tracking-[0.08em] text-[#8993a4] uppercase">Bảng gần đây</p>
        <button className={navButtonClasses} type="button"><span className="w-5 text-center">●</span>Website tuyển dụng</button>
        <button className={navButtonClasses} type="button"><span className="w-5 text-center">○</span>Marketing Q3</button>
      </nav>
